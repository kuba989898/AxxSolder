...
#define BTN_LONG_LONG_PRESS 40 //*50ms (htim16 interval) --> 40 = 2000ms
...
volatile uint8_t SW_1_pressed_long_long = 0;
...
#define GRAPH_Color_TEMP    C_FIREBRICK        // Temperature graph color
#define GRAPH_Color_POWER   C_LAWN_GREEN   // Power graph color

#define DASH_LEN_COLOR   C_DARK_GRAY   // Coordinate grid dashed line color
...
void draw_graph_init(void) {
    ...
    for (int i = 0; i <= 5; i++) {
        ...
        LCD_PutStr(x_draw, y_pos, buf, FONT_arial_19X22, C_PALE_GREEN, background);
    }
...
void draw_axis(const int *values, int count, int x, bool is_left_axis,
    ...
    for (int i = 0; i < count; i++) {
        ...
        LCD_PutStr(x_pos, y_pos - 15, buf, FONT_arial_19X22, color, background);
    }
}
...
void draw_axis_labels(void) {
    ...
    uint16_t color_power_label = (sensor_values.current_state == RUN) ? GRAPH_Color_POWER : C_BLUE;
...
void draw_graph_update(void) {
    ...
    for (uint8_t i = 1; i < GRAPH_POINTS; i++) {
        ...
        uint16_t color_power = (sensor_values.current_state == RUN) ? GRAPH_Color_POWER : C_BLUE;
...
void update_display(){
	...
	if((flash_values.screen_rotation == 0) || (flash_values.screen_rotation == 2)){
		...
		if((sensor_values.current_state == SLEEP || sensor_values.current_state == EMERGENCY_SLEEP || sensor_values.current_state == HALTED) && !sleep_state_written_to_LCD){
			...
			LCD_PutStr(214, 73,  " ", FONT_arial_20X23, RGB_to_BRG(C_BLACK), RGB_to_BRG(C_ORANGE));
			LCD_PutStr(216, 99,  "S", FONT_arial_20X23, RGB_to_BRG(C_BLACK), RGB_to_BRG(C_ORANGE));
			LCD_PutStr(214, 129, "L", FONT_arial_20X23, RGB_to_BRG(C_BLACK), RGB_to_BRG(C_ORANGE));
			LCD_PutStr(216, 158, "E", FONT_arial_20X23, RGB_to_BRG(C_BLACK), RGB_to_BRG(C_ORANGE));
			LCD_PutStr(214, 191, "E", FONT_arial_20X23, RGB_to_BRG(C_BLACK), RGB_to_BRG(C_ORANGE));
			LCD_PutStr(216, 217, "P", FONT_arial_20X23, RGB_to_BRG(C_BLACK), RGB_to_BRG(C_ORANGE));
			LCD_PutStr(214, 247, " ", FONT_arial_20X23, RGB_to_BRG(C_BLACK), RGB_to_BRG(C_ORANGE));
		...
		if((sensor_values.current_state == SLEEP || sensor_values.current_state == EMERGENCY_SLEEP || sensor_values.current_state == HALTED) && !sleep_state_written_to_LCD){
			...
			LCD_PutStr(16, 68,  "L", FONT_arial_20X23, RGB_to_BRG(C_BLACK), RGB_to_BRG(C_ORANGE));
			LCD_PutStr(14, 95,  "E", FONT_arial_20X23, RGB_to_BRG(C_BLACK), RGB_to_BRG(C_ORANGE));
			LCD_PutStr(16, 122, "E", FONT_arial_20X23, RGB_to_BRG(C_BLACK), RGB_to_BRG(C_ORANGE));
			LCD_PutStr(14, 149, "P", FONT_arial_20X23, RGB_to_BRG(C_BLACK), RGB_to_BRG(C_ORANGE));
			LCD_PutStr(16, 176, " ", FONT_arial_20X23, RGB_to_BRG(C_BLACK), RGB_to_BRG(C_ORANGE));
			sleep_state_written_to_LCD = 1;
			standby_state_written_to_LCD = 0;
		}
		else if((sensor_values.current_state == STANDBY) && !standby_state_written_to_LCD){
...
void handle_button_status(){
  ...
  if(SW_1_pressed_long == 1){
	...
	if(SW_1_pressed_long_long == 1){
		SW_1_pressed_long_long=0;
		// start power graph
		draw_pwr_graph=1;
		 //TODO:J.K:Add drawing power graph LONG LONG press SW1
	}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
	...
	if (debounceDone == 1 && htim == &htim16 && SW_ready == 0){
		...
		if(btnPressed == SW_1_Pin && HAL_GPIO_ReadPin(SW_1_GPIO_Port, SW_1_Pin) == GPIO_PIN_RESET){
			...
			beep(flash_values.buzzer_enabled);
			timerCycles = 0;
		}else if(timerCycles > BTN_LONG_PRESS && timerCycles < BTN_LONG_LONG_PRESS && btnPressed == SW_1_Pin && HAL_GPIO_ReadPin(SW_1_GPIO_Port, SW_1_Pin) == GPIO_PIN_RESET){
			SW_ready = 1;
			beep(flash_values.buzzer_enabled);
			SW_1_pressed_long = 1;
			timerCycles = 0;
		}
		else if(timerCycles > BTN_LONG_LONG_PRESS && btnPressed == SW_1_Pin && HAL_GPIO_ReadPin(SW_1_GPIO_Port, SW_1_Pin) == GPIO_PIN_SET){
			SW_ready = 1;
			beep(flash_values.buzzer_enabled);
			SW_1_pressed_long_long = 1;
			timerCycles = 0;
			//TODO:J.K:Add handler to LONG LONG press of SW1
		}
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	...
	while (1){
        ...
        //TODO:JK Add handler to display power graph
        draw_graph_update();
		}
